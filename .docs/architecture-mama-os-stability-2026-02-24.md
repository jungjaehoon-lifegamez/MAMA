...
class ToolRegistry {
  ...
  /** Get tools filtered by allowed list (FR-008: per-agent tool filtering) */
  getFilteredTools(allowedTools?: string[]): ToolDefinition[];

  /** Generate gateway-tools.md content (for LLM prompt) */
  generatePrompt(allowedTools?: string[]): string;

  /** Generate fallback prompt (minimal, for when .md file is missing) */
  generateFallbackPrompt(allowedTools?: string[]): string;
...
**Per-Agent Tool Filtering (FR-008):**

Community feedback identified that even with subprocess isolation (preventing *repeated* injection), agents still pay token cost for *unnecessary* tool descriptions. The `allowed_tools` field per agent config addresses this.

```typescript
// Agent persona config extension
interface AgentPersonaConfig {
  // ... existing fields ...
  allowed_tools?: string[];  // e.g., ['search', 'save', 'update']
}

// Integration in AgentLoop / system prompt builder:
const agentTools = agent.allowed_tools
  ? toolRegistry.getFilteredTools(agent.allowed_tools)
  : toolRegistry.getFilteredTools();  // all tools (backward compat)

const toolPrompt = toolRegistry.generatePrompt(agent.allowed_tools);
```

**Filtering behavior:**
- `allowed_tools` omitted → all tools available (current behavior, backward compatible)
- `allowed_tools: ['search', 'save']` → only those tool descriptions in system prompt
- Hard enforcement: `GatewayToolExecutor.execute()` rejects tool calls not in `allowed_tools`
- Token savings tracked as `tools_filtered_count` metric

**Token impact estimate:**
- Full MAMA MCP tool catalog: ~3K-5K tokens in system prompt
- Typical filtered agent (3-4 tools): ~500-800 tokens
- Per-agent savings: ~2K-4K tokens/turn
...
6. AgentLoop reads `allowed_tools` from agent config and passes to registry

**FRs addressed:** FR-004, FR-008
...
| `tools_filtered_count` | ToolRegistry | Agent prompt built with filtered tools |
...
| FR-008 Tool Filtering | ToolRegistry + AgentLoop | tool-registry.ts, agent-loop.ts, gateway-tool-executor.ts |
