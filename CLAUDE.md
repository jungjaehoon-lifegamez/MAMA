...
- **Standalone Memory Runtime** (`packages/standalone/src/memory/`): SaveCandidateExtractor (rule-based candidate detection), AuditTaskQueue (serial job queue), memory-agent-ack (candidate-aware ack classification), memory-agent-dashboard (stats + channel tracking), recall-bundle-formatter, scope-context (deriveMemoryScopes), agent-notice-queue, bootstrap-context
