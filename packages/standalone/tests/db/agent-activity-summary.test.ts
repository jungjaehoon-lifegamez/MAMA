...
describe('STORY-V019 - Agent activity summary', () => {
  describe('Acceptance Criteria', () => {
    ...
    it('limits latest and recent status calculations to the requested since window', () => {
      logActivity(db, {
        agent_id: 'a1',
        agent_version: 1,
        type: 'task_error',
      ...
      db.prepare(
        "UPDATE agent_activity SET created_at = '2000-01-01 00:00:00' WHERE agent_id = 'a1'"
      ).run();
      logActivity(db, {
        agent_id: 'a1',
        agent_version: 1,
      ...
      const summary = getActivitySummary(db, '2099-01-01');
      expect(summary).toEqual([]);
      const currentWindow = getActivitySummary(db, '2000-01-02');
      const a1 = currentWindow.find((s) => s.agent_id === 'a1')!;
