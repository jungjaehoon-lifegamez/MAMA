...
describe('entity audit handler', () => {
  ...
  it('POST /audit/run returns 202 with run_id', async () => {
  ...
  it('GET /audit/runs lists runs ordered by created_at DESC', async () => {
  ...
  it('GET /audit/runs/:id returns 404 for unknown run', async () => {
    const queue = new EntityAuditRunQueue({ adapter: getAdapter() });
    const handler = createEntityAuditHandler({ queue });
    const res = createMockResponse();
  ...
  it('GET /audit/runs/:id returns run detail + parsed metrics', async () => {
    const queue = new EntityAuditRunQueue({ adapter: getAdapter() });
    const handler = createEntityAuditHandler({ queue });
