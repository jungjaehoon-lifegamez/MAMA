...
describe('Story M1.2: Envelope migration and store contract', () => {
  describe('M1.2 - Envelope migration - AC1: migration table creation', () => {
  ...
  describe('M1.2 - EnvelopeStore - AC1: signed envelope persistence', () => {
  ...
  describe('M1.2 - EnvelopeStore - AC2: idempotency and constraint diagnostics', () => {
  ...
  describe('M1.2 - EnvelopeStore - AC3: readback diagnostics and edge cases', () => {
    it('reports corrupted JSON field with envelope_hash', () => {
      ...
      try {
        ...
        expect(() => store.getByHash(env.envelope_hash)).toThrow(env.envelope_hash);
      } finally {
        closeDb(db, dir);
      }
    });
    ...
    it('rejects corrupted tier values with envelope_hash', () => {
      ...
      try {
        ...
        db.prepare('UPDATE envelopes SET tier = ? WHERE envelope_hash = ?').run(
          4,
          env.envelope_hash
        );
