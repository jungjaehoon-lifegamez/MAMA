...
describe('P2a principal registry completion matrix (TG-04)', () => {
  ...
  it('TG-04 preserves external lane access and connector identity isolation', () => {
    ...
    for (const scenario of scenarios) {
      ...
      const row = repository.resolveByExternal(scenario.connector, scenario.namespace, externalId);
  ...
  it('TG-04 projects all five member tools and refuses them for a non-owner executor role', async () => {
    ...
    const executor = ownerExecutor(repository);
    const codeActNames = new HostBridge(executor).getAvailableFunctions(2).map((tool) => tool.name);
  ...
  it('TG-05 keeps the session key stable when only member principalId changes', () => {
