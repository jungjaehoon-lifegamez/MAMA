...
describe('Telegram owner bookkeeping backfill', () => {
  it('creates one owner row for an unambiguous owner and returns exists on the next boot', () => {
  ...
  it('TG-04 keeps a backfilled owner row bookkeeping-only after owner_user_ids becomes empty', () => {
    const registry = new InMemoryPrincipalRegistry();
    const logger = createLogger();
