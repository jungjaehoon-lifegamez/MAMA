...
describe('isStandaloneDaemonCommand', () => {
  it('matches direct standalone daemon node command', () => {
    expect(
      isStandaloneDaemonCommand(
        '/opt/homebrew/Cellar/node/25.8.0/bin/node /Users/mama-dev/project/MAMA/packages/standalone/dist/cli/index.js daemon'
      )
    ).toBe(true);
  });
  ...
  it('does not match unrelated mama-server processes', () => {
    expect(
      isStandaloneDaemonCommand(
        'node /Users/mama-dev/.codex/vendor/mama-mcp-node-sqlite/node_modules/.bin/mama-server'
      )
    ).toBe(false);
  });
});
...
describe('isStandaloneWatchdogCommand', () => {
  ...
  it('matches legacy inline watchdog command', () => {
    expect(
      isStandaloneWatchdogCommand(
        'node -e const DAEMON_CMD = "/Users/mama-dev/project/MAMA/packages/standalone/dist/cli/index.js"; function checkHealth() {}'
      )
    ).toBe(true);
  });
