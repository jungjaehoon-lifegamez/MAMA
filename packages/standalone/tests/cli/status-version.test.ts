...
interface VersionStatusModule {
  ...
  fetchRuntimeVersion?: (options?: { port?: number; timeoutMs?: number }) => Promise<string | null>;
}
...
describe('Story TG-06: mama status package versions', () => {
  describe('AC: reports CLI and daemon version authority safely', () => {
    ...
    it('uses a wall-clock deadline that trickled bytes cannot extend', async () => {
      await withRuntimeServer(
        ...
        async (port) => {
          expect(
            await resolveWithin(fetchRuntimeVersion?.({ port, timeoutMs: 80 }), 400)
          ).toBeNull();
        }
      );
    });
