  ...
  describe('memory v2 tools', () => {
    it('should route mama_add through ingestMemory instead of fact JSON parsing', async () => {
      ...
      executor.setAgentContext({
        ...
        session: {
          sessionId: 'test-session',
          channelId: 'telegram_test_channel',
          userId: 'telegram_test_channel',
          startedAt: new Date(),
        },
      ...
      expect(mockApi.ingestMemory).toHaveBeenCalledWith(
        expect.objectContaining({
          scopes: expect.arrayContaining([
            expect.objectContaining({ kind: 'channel', id: 'telegram:telegram_test_channel' }),
            expect.objectContaining({ kind: 'user', id: 'telegram_test_channel' }),
          ]),
        })
      );
