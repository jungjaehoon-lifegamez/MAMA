...
export const WIKI_TURN_CONTRACT: readonly string[] = [
  ...
  '- native owner task movement: task_list({view:"items", updated_since: <the payload taskUpdatedSince string>}) — updated_since must be that exact RFC 3339 string (task_list rejects the numeric range.start_ms), and walk nextCursor until the reported total is covered.',
