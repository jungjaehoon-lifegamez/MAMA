...
export interface RegistryPort {
  resolveAlias(alias: string, kind?: string): RegistryNodeView | null;
  createNode(input: {
    kind: string;
    name: string;
  ...
  listNodes(filter?: { kind?: string; parentId?: string | null }): RegistryNodeView[];
...
export async function handleRegistryLookup(
  ...
  try {
    const node = registry.resolveAlias(name, input.kind);
    if (!node) {
    ...
    const children = registry.listNodes({ kind: node.kind, parentId: node.id });
    return {
      success: true,
      found: true,
      node: {
...
export async function handleRegistryUpsert(
  ...
  try {
    const existing = registry.resolveAlias(name, kind);
    if (existing) {
      const added: string[] = [];
      for (const alias of aliases) {
        // Each alias is attached on its own: a conflict on one must not discard the ones
        // already recorded, and the agent needs to know which spelling is contested.
        registry.addAlias(existing.id, alias);
        added.push(alias);
      }
      return { success: true, id: existing.id, created: false, name: existing.name, added };
    }
    const id = registry.createNode({
      kind,
      name,
      aliases,
      note: typeof input.note === 'string' ? input.note : null,
    });
    if (input.parent_of && input.parent_of.length > 0) {
      const children = registry.splitNode({
        parent: id,
        children: input.parent_of,
        reason: typeof input.note === 'string' && input.note ? input.note : 'declared with parent',
      });
      return { success: true, id, created: true, name, children };
    }
    return { success: true, id, created: true, name, added: aliases };
  } catch (error) {
    return { success: false, code: errorCode(error) ?? 'registry_error', error: message(error) };
  }
}
