...
- **task_temporal_reconcile**(expected_revision (required), outcome (resolved|final_no_update|deferred), reason (required), status? or due_at? for resolved, evidence_summary for final_no_update, next_temporal_check_at for deferred) — Resolve, finalize without a lifecycle change, or defer the host-issued temporal work item. Task, generation, occurrence, check, and attempt identity come only from trusted runtime context.
