...
- **report_request**() — Trigger the operator FULL situation report on demand. It is generated by the report machinery (fresh session, delta-anchored) and DELIVERED to the owner channel when done: reply with a short ack, never wait for it or fabricate its contents.
- **board_read**() — Read the owner dashboard report slots (briefing, action_required, decisions, pipeline) with their update times. PRIMARY source for status questions - cite it instead of reconstructing state from memory.
- **audit_findings_read**() — Read the latest deterministic system-audit findings and pass items.
