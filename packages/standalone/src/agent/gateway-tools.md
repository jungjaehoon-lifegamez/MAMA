...
- **report_publish**(slots: { briefing?: html, action_required?: html, decisions?: html }, basis_revision? (currentBasisRevision observed through board_read when reading the analysis evidence; do not fetch a newer basis only to label old analysis current)) — Publish dashboard analysis as HTML. pipeline is a managed live task projection; change tasks instead. Supply the task basis actually used for analysis; omission means its basis is unknown.
...
- **code_act**(code, allowedTools?, blockedTools?) — Execute JavaScript in sandboxed QuickJS. Code-Act runs a synchronous script. Host function calls settle before returning. Use plain sequential calls with var and make the desired value the last expression. Do not use top-level return, async, await, Promise, Promise.all, or async IIFEs.
