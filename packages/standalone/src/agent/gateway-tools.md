...
- **mama_provenance**(memory_id, scopes?) — Trace a stored memory back to the events it rests on. Returns status resolved, partial or unresolved with a named reason (missing_ref, legacy_unscoped, event_deleted, outside_scope, unsupported_ref, unknown_memory), the model run and context packet behind it, and a bounded excerpt per supporting event. Use it to state what a claim is grounded in - or that it is grounded in nothing.
