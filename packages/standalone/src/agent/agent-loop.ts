...
export class AgentLoop {
  ...
  constructor(
    ...
    if (this.backend === 'codex') {
      ...
      this.persistentCLI = new PersistentCLIAdapter({
        ...
        effort: options.codexEffort as 'low' | 'medium' | 'high' | 'max' | undefined,
   ...
   */
  setReportPublisher(fn: Parameters<GatewayToolExecutor['setReportPublisher']>[0]): void {
  ...
  private async runWithContentInternal(
    ...
    try {
      ...
      const result: AgentLoopResult = {
        ...
        stopReason,
        modelRunId: ownedModelRunId ?? options?.modelRunId ?? null,
        modelRunProvenance: ((ownedModelRunId ?? options?.modelRunId)
          ? 'available'
  ...
  private async executeTools(
    ...
    for (const toolUse of toolUseBlocks) {
      ...
      try {
        ...
        const toolResult: GatewayToolResult = await this.mcpExecutor.execute(
          executionToolName,
          toolUse.input as GatewayToolInput,
