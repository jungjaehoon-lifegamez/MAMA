...
## Tools
- task_list / task_update / task_reclassify -- the NATIVE task ledger, and the pipeline slot's projection source. Start with overview; input.reclassificationCandidates is the host-issued authority for at most ten rows. Read only those details in groups of four. Reconcile candidates stay on the affected channel; full maintenance candidates are one active legacy page. task_reclassify rejects every other id. Never load the whole board in one run. task_create is host-blocked on unattended turns: records and tasks are SEPARATE, so a connector item with no row stays evidence and only an owner conversation creates a task. contract_no_update({reason, scope}) records a judged no-op in reconcile runs.
