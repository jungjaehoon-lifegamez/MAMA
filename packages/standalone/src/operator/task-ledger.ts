 ...
 * enforces revisions, idempotency, and atomic receipts.
 *
...
import { applyOperatorTaskDeadlineMigration } from '../db/migrations/operator-task-deadlines.js';
...
export const WORKORDER_KINDS = ['board', 'wiki', 'memory-curation', 'self-check'] as const;
 ...
 * someone else's run. Same reason a lane takes its identity from trusted runtime
...
export interface ListTasksFilter {
  ...
  priority?: TaskPriority;
  /** Derived due partition. date_due belongs to upcoming. */
  dueBucket?: DueBucket;
 ...
 * due buckets derived from the identical due classification `items` uses,
...
export interface OverviewResult {
  ...
  /**
   * Partition of `total` by due disposition under `observedAt`:
   * missing = no due_at and no deadline; overdue/upcoming from the same
   * deriveDueState `items` uses (date_due counts as upcoming); closed =
...
export class TaskLedger implements TaskSource {
  ...
  dueStateAt(task: TaskRecord, observedAt: number): DueState {
    return deriveDueState(task, observedAt, this.timeZone);
  }
  ...
  private upgradeSchema(): void {
    ...
    try {
      ...
      applyOperatorTaskDeadlineMigration(this.db);
  ...
  private temporalAsOf(value: number | undefined): number {
    ...
    if (!Number.isSafeInteger(observedAt)) {
      throw new Error('task list as-of must be a finite integer epoch millisecond');
    }
    return observedAt;
  }
    ...
    if (filter.dueBucket !== undefined) {
      ...
      const ids = rows
        .filter((row) => {
          ...
          return dueBucketForState(state) === filter.dueBucket;
        })
        .map((row) => row.id);
   ...
   * one JS pass over minimal rows using the SAME deriveDueState `items`
        ...
        .prepare(
          `INSERT INTO operator_tasks
           (title, status, priority, assignee, deadline, due_at, deadline_offset_minutes,
            revision, source_channel, source_event_id, latest_event,
  ...
  reclassify(id: number, input: ReclassifyTaskInput, origin: ChangeOrigin = {}): TaskRecord {
    ...
    try {
      ...
      // revision-checked UPDATE and effect receipt.
  ...
  private assertTaskReclassificationAuthorized(origin: ChangeOrigin): void {
    if (origin.workOrderAttemptId !== undefined) {
      ...
      // wiki run that discovers a record-shaped owner row may
    ...
    const state = deriveDueState(
      {
        ...
        // ask the deadline question about the CLOCK fields only.
        status: 'pending',
