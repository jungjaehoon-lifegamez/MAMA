    ...
    const framing =
      mode === 'full'
        ? [
            ...
            'Use overview aggregates for counts and narrow items by the relevant due bucket, status, channel, or changed-since window. A cursor is available evidence, not an obligation to scan every row.',
            'Reuse evidence already gathered in this conversation while it is current. After your own writes, refresh affected details or aggregate counts; do not traverse the whole board again to recompute a summary.',
            'Conclude when the material owner decisions and next actions are supported. Continue a deeper investigation only if its missing evidence can change those decisions, and state remaining coverage gaps.',
