  ...
  switch (kind) {
    case 'board':
      return [
        ...
        'Read the board progressively. Start with task_list({view:"overview", include_terminal:false}) for shape and counts. In reconcile mode read only the affected channel/current source and do not run legacy cleanup. In full/clock maintenance, read ONE small legacy page with task_list({view:"items", qualification:"legacy_unqualified", include_terminal:false, limit:10}); inspect selected rows with task_list({view:"detail", ids:[...]}) and stop after that page. Do not walk every cursor or load the whole board in one run. A real legacy task leaves this queue when task_update adds a concrete completion_criteria; a record/memory/completed item leaves it through task_reclassify. Compare only selected rows against the relevant live sources: trello_kanban/trello_search/trello_card for Trello and context_compile for connector messages or the polled delta. Your judgment decides what is the same work, what is finished, what is stale and what is unknown. Merge duplicates, close what is done, and put what you cannot decide in the decisions slot with the evidence, options and recommendation; do not wait for an answer.',
