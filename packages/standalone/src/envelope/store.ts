...
export class EnvelopeStore {
  ...
  insert(env: Envelope): void {
    ...
    try {
      ...
      if (this.isUniqueConstraintError(error)) {
        ...
        if (
          existing &&
          JSON.stringify(this.envelopeToParams(existing)) === JSON.stringify(params)
        ) {
          return;
        }
      }
  ...
  private envelopeToParams(env: Envelope): Record<string, unknown> {
    return {
      ...
      trigger_context: JSON.stringify(env.trigger_context),
      scope: JSON.stringify(env.scope),
      tier: env.tier,
      budget: JSON.stringify(env.budget),
      expires_at: env.expires_at,
      signature: JSON.stringify(env.signature),
    };
  }
