...
describe('channel summary state reducer', () => {
  ...
  it('should accumulate active decisions and milestones instead of overwriting the whole summary', async () => {
    await recordChannelAudit({
      ...
      scopeRefs: [{ kind: 'channel', id: 'telegram:telegram_test_channel' }],
      ack: {
        status: 'applied',
        action: 'save',
        event_ids: [],
    ...
    const state = await getChannelSummaryState('telegram:telegram_test_channel');
    const summary = await getChannelSummary('telegram:telegram_test_channel');
