-- Extend twin_edges.edge_type with the judgment-boundary relations 'refines',
-- 'contradicts', and 'amends'. Authored record links use these relations; the
-- 074 shape predates them and rejects the inserts with a CHECK violation.
-- Rebuild the table like 074 did, preserving every row and column.

ALTER TABLE twin_edges RENAME TO twin_edges_legacy_079;
...
INSERT INTO twin_edges SELECT * FROM twin_edges_legacy_079;
DROP TABLE twin_edges_legacy_079;
CREATE INDEX idx_twin_edges_subject ON twin_edges(subject_kind, subject_id, created_at DESC);
CREATE INDEX idx_twin_edges_object ON twin_edges(object_kind, object_id, created_at DESC);
CREATE INDEX idx_twin_edges_model_run_id ON twin_edges(model_run_id, created_at DESC);
...
VALUES (79, 'Twin edge relations for judgment links (refines/contradicts/amends)');
