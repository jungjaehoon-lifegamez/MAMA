...
## Report improvement candidate: 2026-09-07 (not released)

- **TG-01/TG-04/TG-05/TG-06:** inbound owner turns remain ordered, but no longer hold the
  outbound batch lock while waiting for the owner model. Ready reports can be delivered during
  unrelated model work. Each final multipart answer/send holds one chat delivery lease; streaming
  edits drain before the final batch acquires it. A completed lease cannot authorize detached
  reentry, and live ready responses remain owned by their presenter rather than periodic recovery.
- **TG-05:** LaneManager restores each task's admission async context and preserves priority
  through the global queue. On-demand report intent receives owner priority while scheduled
  reports remain background work; active and earlier equal-priority owner work is preserved.
- **TG-06:** pending/retry delivery no longer emits SENT or returns successful delivery to the
  tick result. Exact durable receipt, target/payload identity and replay behavior remain in place.
- **TG-03/TG-06:** schedule_upcoming now includes ongoing intervals as an explicit extension of
  its former future-start contract. Calendar changes become immutable version observations; the
  reader selects the latest event before filtering cancellation/time, computes zoned all-day
  boundaries, and returns coherent cursor pages with source/freshness limitations. Historical raw
  data still cannot prove upstream completeness.
- **Discovery evidence (synthetic stores; not a release gate):** real Telegram callback -> MessageRouter -> boot AgentLoopClient ->
  Codex app-server protocol -> CodeAct -> owner effect ledger -> gateway transport/receipt is
  exercised with synthetic stores and external model/Telegram doubles. The baseline reproduction
  fails before the queue correction; candidate background-first and queued-background after an
  inbound turn preserve delivery progress and contiguous multipart sends.
- **Tests:** telegram-owner-queue.integration.test.ts, telegram.test.ts,
  telegram-response-presenter.test.ts, lane-manager.test.ts, report-run.test.ts,
  operator-trigger-loop-report-delivery.test.ts, schedule-upcoming.test.ts, calendar.test.ts.
  Actual model judgment/retrieval comparisons and consolidated gates are recorded in
  [report-improvement-validation.md](report-improvement-validation.md). No installed-runtime
  improvement or percentile SLO is claimed by this candidate section.
...
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| TG-01 | Messages in one Telegram conversation are handled and presented in order, including long multi-part replies.                                                                                        | `channels/telegram/telegram-channel.ts`: `enqueueStream`, `handleStreaming`, `splitMessage`                                                                                                                                                                             | `gateways/telegram.ts`, `gateways/telegram-message-ledger.ts`, `gateways/telegram-response-presenter.ts`, `gateways/message-router.ts`, `operator/report-carry-delivery.ts`, `operator/pending-report-store.ts`, `operator/operator-trigger-loop.ts`                                | MAMA serializes inbound processing per principal lane and outbound multipart batches per chat. Waiting for model admission never holds outbound delivery; ready external reports can deliver while a same-chat model turn is active. Reentry requires the current live outbound lease, not inherited conversation ownership. The production report assembly forwards the prepared delivery ID to one host-bound Telegram target. Pending requests and deliveries persist that target plus a payload identity, and recovery rejects configuration drift or mutated payloads before send or carry persistence. Telegram binds one delivery ID to the same chat and text: conflicting reuse is rejected, identical reuse stays idempotent. Existing confirmed/uncertain chunk semantics remain unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `telegram.test.ts`: exact external report delivery during active same-chat model work, conflicting chat/text reuse, overlapping turns, failed chunk, 429, Unicode boundary. `operator-trigger-loop.test.ts`: scheduled/on-demand target-scoped production assembly and cross-boot target drift. `pending-report-store.test.ts`: target/payload validation and quarantine. `telegram-response-presenter.test.ts`: long response and Unicode boundary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | GREEN (2026-08-02, historical; superseded by the 2026-09-07 candidate section above) |
...
- **Status:** CODE GREEN; v0.39.0 release cutover is complete, and a real owner Telegram feedback
...
- **Status:** CODE GREEN, merged as PR #230, and released in v0.39.0; a real owner-event canary
  remains pending.
...
- 2026-08-26: TG-05 now carries committed owner-report receipt history as a separate AgentLoop
  ...
  exact history block is removed whole before the Code-Act catalog. Focused replacement-session
  and prompt-budget regressions cover the structured boundary; live evidence waits for the next
  remediation release canary.
...
- 2026-08-26: Recorded TG-03/TG-04/TG-05/TG-06 code evidence for durable exact-batch Board
  ...
  PR #230 and PR #231 are merged and released in v0.39.0. The release was installed and cut over
  to one healthy launchd-owned daemon; real owner-event and Temporal canaries remain pending.
...
## 2026-09-07 — TG-05/TG-06 공통 보고 실행 계약 전환 중

- full/digest를 `PersonaReportAsk.compose(ReportRunInput)`으로 통합했다. 보고 발생 참조와
  종류가 필수이며, 모두 모델 실행 전에 요청을 저장한다.
- 문자열 실행 경로, authoring 호출로의 암묵적 대체, 과거 assistant 문장을 최종 보고로
  승격하는 폴백을 제거했다. 전송 재시도는 기존 저장된 본문을 사용한다.
