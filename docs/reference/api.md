...
The authenticated native task ledger is exposed separately by
`operator-tasks-handler.ts`:

- `GET /api/operator/tasks` — list tasks in deadline/priority order. Optional query fields are
  `status`, `source_channel`, and `limit` (1-200, default 50).
...
is returned in `deadline_offset_minutes`. Conflicting `due_date` and `due_at` values are rejected.

PATCH clear/update combinations are intentional:
...
| `context_key`   | string   | No       | Internal host-issued 32-byte base64url key for one managed process generation. External clients must not supply or persist this key |
...
`allowed_tools`/`blocked_tools`, and can be forced into read-only Code-Act mode with
`MAMA_CODE_ACT_READ_ONLY=true`. No `require()`, `process`, or `fs` APIs are available.

Managed persistent-Claude MCP calls include a fresh `context_key` generated by the host for the
exact process generation. When that key names a live prompt lease, the endpoint reacquires the
host-trusted principal and cancellation context; request identity fields cannot replace it. An
unknown, expired, or closing key returns `CODE_ACT_CONTEXT_UNAVAILABLE` with `retryable: false`.
Direct API clients should omit `context_key` and use the `agent_id` policy path. Treat host-issued
keys as ephemeral secrets and never log, persist, or copy them into configuration.
