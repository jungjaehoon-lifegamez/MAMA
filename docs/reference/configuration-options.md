...
| `~/.mama/config.yaml` | MAMA OS daemon              | YAML   | Everything else: gateways, connectors, multi-agent personas, timeouts, and report hours. The former top-level stateful `conductor` block is retired and ignored; an optional multi-agent persona named `conductor` is a separate legacy concept. Managed by `mama init`/setup; schema in `packages/standalone/src/cli/config/types.ts`. |
