...
| `MAMA_TEMPORAL_RECONCILE`      | — (env only)    | `export MAMA_TEMPORAL_RECONCILE=on` — temporal owner-task reconciliation, `off\|on`, default `off`. `on` requires `MAMA_STAGE2_WORKORDERS=on`, a Claude/Codex backend, the trusted `task_temporal_reconcile` transport tool, and the Stage-2 consumer. Malformed or incompatible values fail boot before the scanner starts; `off` pauses open temporal attempts for safe later resume.                   |
